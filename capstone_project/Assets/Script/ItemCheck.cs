...
public class ItemCheck : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D Item)
    {
        if (Item.gameObject.CompareTag("Item"))      //아이템 체크
        {
            ...
            {
                Item i = Item.gameObject.GetComponent<Item>();
                transform.parent.GetComponent<PlayerCharacter>().get_item.Add(new ItemClass(i.get_item(), i.get_rarity()));
                Destroy(Item.gameObject);
                /*if (Item.GetComponent<Item>().ItemType == 1true)        //액티브
                {
                ...
                else if (Item.GetComponent<Item>().ItemType == 2true)        //패시브
                {
